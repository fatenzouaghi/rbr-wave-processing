...
%               HAB  (required, m)  – sensor Height Above Bed = zmembrane - zfond
...
% Hydrostatic water level (for information): level = p/(ρg) + HAB
level = p./(rho*g) + opts.HAB;

%% 3) Pressure spectrum (your original function), then convert to elevation spectrum
...
h_mean = mean(p)/(rho*g) + opts.HAB;   % approximate mean depth used as 'd'
...
if opts.use_attenuation
    k  = wavek(ff, h_mean, g);
    TF = cosh(k.*h_mean) ./ cosh(k.*opts.HAB) / (rho*g);
